...
void Taster::loop()
{
    if (SI::get().config().tasterFunc != TASTER_NONE) {
        ...
        if (value > 0) {
            ...
            if (_pressDetected) {
                // execute action
                switch (SI::get().config().tasterFunc) {
                case TASTER_CHECK_NOW:
                    I::get().logger() << "[Taster] trigger check now" << endl;
                    // SI::get().shellyEm3Connector().sleeper().wakeup();
                    break;
                default:
                    I::get().logger() << "[Taster] undefined action: " << SI::get().config().tasterFunc << endl;
                    break;
                }
                _pressDetected = false;
            }
