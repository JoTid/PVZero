...
window.addEventListener("DOMContentLoaded", function (event) {
  ...
  //     timerRefresh = setTimeout(getJSON.bind(null, "/pvz/state.json", "fillPVZeroState"), 1000);
  // }, false);

  setTimeout(getJSON.bind(null, "/pvz/state.json", "fillPVZeroState"), 500);
});
...
function fillPVZeroState(data, url) {
  ...
  document.getElementById("mppt_state").innerText =
    "MPPT Status: " + data["mppt_state"];
  document.getElementById("battery_state").innerText =
    "PVZero Status: " + data["battery_state"];
